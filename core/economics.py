...
class PlatformEconomics:
    ...
    # 5% of platform commission goes to rewards
    TEOCOIN_REWARD_POOL_RATE = Decimal("0.05")
    TEOCOIN_EURO_EXCHANGE_RATE = Decimal("0.50")  # 1 TEO = €0.50 for discounts

    # DISCOUNT LIMITS
    # Max 20% discount per course
    MAX_DISCOUNT_PERCENTAGE = Decimal("0.20")
    ...
    # 5% of course price as TEO reward
    COURSE_PURCHASE_REWARD_RATE = Decimal("0.05")
    ...
    @classmethod
    def get_discount_with_teo(
        cls, course_price_eur: Decimal, teo_to_spend: Decimal
    ) -> dict:
        """
    ...
    @classmethod
    def compare_annual_earnings(
        cls, monthly_sales: int, avg_course_price: Decimal
    ) -> dict:
        """
        ...
        return {
            ...
            "advantage_vs_skillshare_eur": float(
                schoolplatform_annual - skillshare_annual
            ),
            ...
            "advantage_vs_skillshare_percent": float(
                cls.OUR_ADVANTAGE_VS_SKILLSHARE * 100
            ),
    ...
    def calculate_staking_economics(
        cls, course_price_eur: Decimal, teacher_teo_staked: Decimal = Decimal("0")
    ) -> dict:
        """

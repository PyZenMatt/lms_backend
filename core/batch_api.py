...
class StudentBatchDataAPI(APIView):
    ...
    def get(self, request):
        ...
        progress_data = UserProgressSerializer(
            user_progress, context={"request": request}
        ).data
        ...
        recent_completions = (
            LessonCompletion.objects.filter(student=user)
...
class CourseBatchDataAPI(APIView):
    ...
    def get(self, request, course_id):
        ...
        data = {
            ...
            "enrollment": {"is_enrolled": is_enrolled, "can_access": True},
        }
        ...
        try:
            ...
            if lesson.course and not lesson.course.is_approved:
                if not (
                    ...
                    or lesson.course.teacher == request.user
                ):
            ...
            if lesson.course:
                ...
                if not is_enrolled and lesson.course.teacher != request.user:
                    return Response(
                        {"error": "Not enrolled in this course"}, status=403
                    )
            ...
            exercises_data.append(
                {
                    ...
                    "difficulty": (
                        exercise.difficulty if hasattr(exercise, "difficulty") else None
                    ),
                }
            )

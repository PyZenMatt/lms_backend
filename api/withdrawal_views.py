            ...
            if result["success"]:
                ...
                if mint_result["success"]:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error updating withdrawal #{withdrawal_id} status: {e}"
                        )
                        # Return original pending response if status update fails

                else:
                    logger.error(
                        f"❌ Auto-processing failed for withdrawal #{withdrawal_id}: {mint_result.get('error')}"
                    )
    ...
    def get(self, request):
        ...
        try:
            ...
            withdrawals = teocoin_withdrawal_service.get_user_withdrawal_history(
                user=request.user, limit=limit, status=status_filter
            )

            logger.info(
                f"✅ Retrieved {len(withdrawals)} withdrawals for {request.user.email}"
            )
        ...
        try:
            ...
            limits = {
                ...
                "daily_usage": {
                    ...
                    "withdrawals_remaining": max(
                        0,
                        teocoin_withdrawal_service.MAX_DAILY_WITHDRAWALS - daily_count,
                    ),
                    "amount_remaining": str(
                        max(
                            0,
                            teocoin_withdrawal_service.MAX_DAILY_AMOUNT - daily_amount,
                        )
                    ),
                },
            }
            ...
            return Response(
                {"success": True, "limits": limits}, status=status.HTTP_200_OK
            )

        except Exception as e:
            logger.error(
                f"Error getting withdrawal limits for {request.user.email}: {e}"
            )
            return Response(
                {
                    "success": False,
...
class ProcessWithdrawalView(APIView):
    ...
    def post(self, request, withdrawal_id):
        ...
        try:
            ...
            try:
                withdrawal = TeoCoinWithdrawalRequest.objects.get(
                    id=withdrawal_id, user=request.user, status="pending"
                )
            ...
            logger.info(
                f"🎯 Processing withdrawal #{withdrawal_id} for user {request.user.email}"
            )
            ...
            if result["success"]:
                ...
                logger.error(
                    f"❌ Withdrawal #{withdrawal_id} failed: {result.get('error')}"
                )
...
class ProcessPendingWithdrawalsView(APIView):
    ...
    permission_classes = [
        # For now, authenticated users can trigger (for testing)
        IsAuthenticated
    ]
    ...
    def post(self, request):
        ...
        try:
            logger.info(
                f"Processing pending withdrawals triggered by {request.user.email}"
            )
...
class MintToAddressView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            result = teocoin_withdrawal_service.mint_tokens_to_address(
                amount=amount_decimal, to_address=to_address
            )

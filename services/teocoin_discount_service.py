...
class TeoCoinDiscountService:
    ...
    def create_discount_request(
        ...
        try:
            ...
            try:
                request_id_before = (
                    self.discount_contract.functions.getCurrentRequestId().call()
                )
                self.logger.info(
            ...
            try:
                ...
                contract_platform_account = (
                    self.discount_contract.functions.platformAccount().call()
                )
                self.logger.info(
                ...
                try:
                    ...
                    current_allowance = (
                        self.teocoin_service.contract.functions.allowance(
                            student_checksum, discount_contract_checksum
                        ).call()
                    )
                    ...
                    try:
                        ...
                        student_balance = (
                            self.teocoin_service.contract.functions.balanceOf(
                                student_checksum
                            ).call()
                        )
                        self.logger.info(
                ...
                try:
                    ...
                    if (
                        "TEO deduction from student failed" in error_msg
                        or "transferFrom" in error_msg
                    ):
                        self.logger.warning(
            ...
            if receipt.status == 0:
                try:
                    ...
                    try:
                        function_call.call({"from": self.platform_account.address})
            ...
            result = {
                ...
                + timedelta(hours=self.REQUEST_TIMEOUT_HOURS),
            }
            ...
            try:
                ...
                student_user = User.objects.filter(
                    wallet_address__iexact=student_address
                ).first()
                teacher_user = User.objects.filter(
                    wallet_address__iexact=teacher_address
                ).first()
                ...
                if student_user and teacher_user:
                    ...
                    try:
                        ...
                        commission_rate = (
                            payment_service.get_teacher_commission_rate(teacher_user)
                            * 100
    ...
    def approve_discount_request(self, request_id: int, approver_address: str) -> Dict:
        ...
        try:
            ...
            try:
                student_user = User.objects.filter(
                    wallet_address__iexact=request_data["student"]
                ).first()
                teacher_user = User.objects.filter(
                    wallet_address__iexact=request_data["teacher"]
                ).first()
                ...
                if student_user and teacher_user:
                    ...
                    teocoin_notification_service.notify_student_teacher_decision(
                        ...
                        decision="accepted",
                        teo_amount=(
                            request_data["teo_cost"] + request_data["teacher_bonus"]
                        )
                        / 10**18,
                    )
    ...
    def decline_discount_request(
        self, request_id: int, decliner_address: str, reason: str
    ) -> Dict:
        """
    ...
    def generate_student_signature_data(
        self, student_address: str, course_id: int, teo_cost: int
    ) -> Dict:
        """
    ...
    def calculate_teo_cost(
        self, course_price: Decimal, discount_percent: int
    ) -> Tuple[int, int]:
        """
        Calculate TEO cost and teacher bonus
    ...
    def _calculate_teo_amounts(
        self, course_price_cents: int, discount_percent: int
    ) -> Tuple[int, int]:
        """
    ...
    def _execute_platform_transaction(self, function_call) -> bytes:
        ...
        try:
            ...
            signed_txn = self.w3.eth.account.sign_transaction(
                transaction, private_key=self.platform_account.key
            )
    ...
    def _extract_request_id_from_receipt(self, receipt) -> int:
        ...
        try:
            ...
            for log_index, log in discount_contract_logs:
                ...
                if (
                    len(log.topics) >= 2
                ):  # At least event signature + one indexed parameter
                    try:

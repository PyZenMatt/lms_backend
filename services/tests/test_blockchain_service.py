...
class BlockchainServiceTest(TestCase):
    ...
    def test_link_wallet_invalid_address(self):
        ...
        with patch.object(
            self.service.teocoin_service, "validate_address", return_value=False
        ):
            with self.assertRaises(BlockchainTransactionError):
    ...
    def test_mint_tokens_to_user_success(self):
        ...
        with patch.object(
            self.service.teocoin_service, "mint_tokens", return_value="0xabcdef"
        ) as mock_mint:
            with patch.object(
                self.service, "test_mode", False
            ):  # Force real blockchain call

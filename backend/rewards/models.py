...
class TeoCoinEscrow(models.Model):
    ...
    student = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="teocoin_escrows_as_student"
    )
    teacher = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="teocoin_escrows_as_teacher"
    )
    course = models.ForeignKey(
        "courses.Course", on_delete=models.CASCADE, related_name="teocoin_escrows"
    )

    # Financial details
    teocoin_amount = models.DecimalField(
        max_digits=18, decimal_places=8, help_text="Amount of TeoCoin held in escrow"
    )
    ...
    discount_euro_amount = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Euro amount of discount"
    )
    original_course_price = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Original course price in EUR"
    )
    ...
    def __str__(self):
        return (
            f"Escrow: {self.teocoin_amount} TEO for {self.course.title} ({self.status})"
        )

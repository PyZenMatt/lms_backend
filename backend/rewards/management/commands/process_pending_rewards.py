    ...
    def handle(self, *args, **options):
        ...
        if total_count == 0:
            self.stdout.write(self.style.SUCCESS(
                "✅ No pending reward transactions to process"))
            return
        ...
        for tx in pending_rewards:
            try:
                ...
                try:
                    ...
                    self.stdout.write(self.style.ERROR(
                        f"❌ Blockchain error: {blockchain_error}"))
                    failed += 1
        ...
        if failed > 0:
            self.stdout.write(self.style.WARNING(
                f"⚠️ {failed} transactions failed to process"))

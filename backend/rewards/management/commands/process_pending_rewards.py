...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if total_count == 0:
            self.stdout.write(
                self.style.SUCCESS("✅ No pending reward transactions to process")
            )
            return
        ...
        if dry_run:
            ...
            for tx in pending_rewards:
                self.stdout.write(
                    f"Would process: {tx.user.username} - {tx.amount} TEO ({tx.transaction_type})"
                )
            return
        ...
        for tx in pending_rewards:
            try:
                ...
                if not tx.user.wallet_address:
                    self.stdout.write(
                        self.style.ERROR(
                            f"❌ User {tx.user.username} has no wallet address"
                        )
                    )
                    tx.status = "failed"
                ...
                try:
                    ...
                    tx_hash = teocoin_service.mint_tokens(
                        tx.user.wallet_address, tx.amount
                    )
                    ...
                    if tx_hash:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(f"✅ Success! TX Hash: {tx_hash}")
                        )
                        processed += 1
                    else:
                        # Mark as failed
                        ...
                        self.stdout.write(
                            self.style.ERROR(f"❌ Failed: No tx hash returned")
                        )
                        failed += 1
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"❌ Blockchain error: {blockchain_error}")
                    )
                    failed += 1
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"❌ Unexpected error processing transaction {tx.pk}: {e}"
                    )
                )
                failed += 1
        ...
        if processed > 0:
            self.stdout.write(
                self.style.SUCCESS(
                    f"🎉 Successfully processed {processed} reward transactions!"
                )
            )
        if failed > 0:
            self.stdout.write(
                self.style.WARNING(f"⚠️ {failed} transactions failed to process")
            )

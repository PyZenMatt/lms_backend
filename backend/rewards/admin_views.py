...
def admin_transactions_list(request):
    ...
    try:
        ...
        # Query base
        queryset = BlockchainTransaction.objects.select_related("user").order_by(
            "-created_at"
        )
...
def admin_transactions_stats(request):
    ...
    try:
        ...
        recent_stats = (
            BlockchainTransaction.objects.filter(created_at__gte=thirty_days_ago)
            .values("transaction_type")
            .annotate(
                count=Count("id"), volume=Sum("amount", filter=Q(status="completed"))
            )
            .order_by("-count")
        )
...
def admin_retry_transaction(request, transaction_id):
    ...
    try:
        ...
        logger.info(
            f"Admin {request.user.username} retrying transaction {transaction_id}"
        )
        ...
        return Response(
            {"error": "Transazione non trovata"}, status=status.HTTP_404_NOT_FOUND
        )
    except Exception as e:
...
def admin_user_transactions(request, user_id):
    ...
    try:
        ...
        transactions = BlockchainTransaction.objects.filter(user=user).order_by(
            "-created_at"
        )[:20]

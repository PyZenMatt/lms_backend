...
from .models import (Achievement, User, UserAchievement, UserProgress,
                     UserSettings)
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'username', 'email', 'first_name', 'last_name', 'phone', 'address',
                  'role', 'avatar', 'bio', 'profession', 'artistic_aspirations', 'wallet_address']
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['username', 'email', 'first_name', 'last_name', 'phone', 'address', 'role',
                  'avatar', 'bio', 'profession', 'artistic_aspirations', 'date_joined', 'wallet_address']
...
class UserSettingsSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if privacy_data:
            instance.show_profile = privacy_data.get(
                'show_profile', instance.show_profile)
            instance.show_progress = privacy_data.get(
                'show_progress', instance.show_progress)
            instance.show_achievements = privacy_data.get(
                'show_achievements', instance.show_achievements)
...
class UserAchievementSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['achievement', 'earned_date',
                  'progress_percentage', 'earned']
...
class UserProgressSerializer(serializers.ModelSerializer):
    ...
    def get_categories(self, obj):
        ...
        category_choices = Course.CATEGORY_CHOICES if hasattr(
            Course, 'CATEGORY_CHOICES') else []
        ...
        for category_code, category_name in category_choices:
            data = category_data.get(
                category_code, {'total_courses': 0, 'completed_courses': 0})
    ...
    def get_achievements(self, obj):
        user_achievements = UserAchievement.objects.filter(
            user=obj.user).select_related('achievement')
    ...
    def get_recent_activities(self, obj):
        ...
        course_completions = CourseEnrollment.objects.filter(
            student=obj.user,
            completed=True,
            # Fixed field name: enrolled_at instead of enrollment_date
            enrolled_at__gte=recent_date

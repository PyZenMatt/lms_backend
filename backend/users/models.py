...
class User(AbstractUser):
    ...
    bio = models.TextField(
        blank=True, null=True, help_text="Breve biografia (opzionale)."
    )
    ...
    role = models.CharField(
        ...
        error_messages={"invalid_choice": '"%(value)s" non è una scelta valida.'},
    )
    ...
    purchased_lessons = models.ManyToManyField(
        "courses.Lesson", related_name="purchasers", blank=True
    )
...
class UserProgress(models.Model):
    ...
    def calculate_overall_progress(self):
        ...
        if self.total_courses_enrolled == 0:
            return 0
        return round(
            (self.total_courses_completed / self.total_courses_enrolled) * 100, 2
        )
...
class TeacherProfile(models.Model):
    ...
    def update_tier_and_commission(self):
        ...
        tier_config = {
            ...
            # 40% platform
            "Gold": {
                "min_stake": Decimal("300.00"),
                "commission_rate": Decimal("40.00"),
            },
            # 35% platform
            "Platinum": {
                "min_stake": Decimal("600.00"),

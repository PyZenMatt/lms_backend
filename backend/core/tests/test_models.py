...
class UserModelTests(TestCase):
    def test_user_creation_with_roles(self):
        ...
        teacher = User.objects.create_user(
            username=f"teacher_{timestamp}", password="testpass123", role="teacher"
        )

        student = User.objects.create_user(
            username=f"student_{timestamp}", password="testpass123", role="student"
        )
...
class LessonModelTests(TestCase):
    ...
    def test_course_purchase_flow(self):
        student = User.objects.create_user(
            username=f"student_{time.time()}", password="testpass123", role="student"
        )
...
class ExerciseModelTests(TestCase):
    def setUp(self):
        self.teacher = User.objects.create_user(
            username=f"ex_teacher_{time.time()}", password="testpass123", role="teacher"
        )

        self.student = User.objects.create_user(
            username=f"learner_{time.time()}", password="testpass123", role="student"
        )
    ...
    def test_teocoin_assignment(self):
        exercise = Exercise.objects.create(
            student=self.student, lesson=self.lesson, status="submitted"
        )

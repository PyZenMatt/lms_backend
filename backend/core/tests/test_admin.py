...
class LessonAdminTest(TestCase):
    def setUp(self):
        self.admin_user = User.objects.create_superuser(
            username="admin", password="adminpass", role="teacher"
        )
        ...
        self.lesson = Lesson.objects.create(
            title="Test Lesson", content="Content", teacher=self.teacher, duration=60
        )
        self.client = Client()
    ...
    def test_teacher_filter_queryset(self):
        ...
        self.assertQuerySetEqual(
            teacher_field.queryset.order_by("id"), teachers, transform=lambda x: x
        )
...
class ExerciseAdminTest(TestCase):
    def setUp(self):
        ...
        self.lesson = Lesson.objects.create(
            title="Test Lesson", content="Content", teacher=self.teacher, price=100
        )
        self.exercise = Exercise.objects.create(
            student=self.student, lesson=self.lesson, submission="Test submission"
        )
        self.client = Client()
        self.client.force_login(self.admin_user)

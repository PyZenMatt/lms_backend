...
def get_analytics_data():
    ...
    paid_courses = Course.objects.filter(
        is_approved=True, price_eur__gt=0).count()
    free_courses = Course.objects.filter(is_approved=True, price_eur=0).count()

    # Enrollment Statistics
    total_enrollments = CourseEnrollment.objects.count()
    fiat_enrollments = CourseEnrollment.objects.filter(
        payment_method='fiat').count()
    teocoin_enrollments = CourseEnrollment.objects.filter(
        payment_method='teocoin').count()
    free_enrollments = CourseEnrollment.objects.filter(
        payment_method='free').count()
    ...
    # Average order value
    aov = (total_revenue_eur /
           fiat_enrollments) if fiat_enrollments > 0 else Decimal('0')

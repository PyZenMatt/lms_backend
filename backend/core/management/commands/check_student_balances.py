...
def check_student_balances():
    ...
    student_matic_wei = teocoin_service.w3.eth.get_balance(
        Web3.to_checksum_address(student_address))
    matic_balance = float(
        teocoin_service.w3.from_wei(student_matic_wei, 'ether'))
    ...
    print(
        f"Student has enough TEO: {prerequisites['student']['has_enough_teo']}")
    print(
        f"Student has enough MATIC: {prerequisites['student']['has_enough_matic']}")
    print(
        f"Reward pool has enough MATIC: {prerequisites['reward_pool']['has_enough_matic']}")

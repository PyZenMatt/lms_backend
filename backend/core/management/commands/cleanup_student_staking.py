...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not students_with_staking:
            self.stdout.write(
                self.style.SUCCESS(
                    "✅ No students found with staking data. System is clean!")
            )
            return

        self.stdout.write(
            self.style.WARNING(
                f"Found {len(students_with_staking)} students with staking data:")
        )
        ...
        for data in students_with_staking:
            ...
            self.stdout.write(
                f"  • {student.email}: {staked_amount} TEO staked")
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"    ✅ Moved {staked_amount} TEO from staked to available")
                )

            total_unstaked += staked_amount

        # Clean up any TeacherProfile records for students
        student_teacher_profiles = TeacherProfile.objects.filter(
            user__role='student')
        ...
        if student_teacher_profiles.exists():
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"Found {profile_count} TeacherProfile records for students")
            )
            ...
            if not dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"    ✅ Deleted {profile_count} invalid TeacherProfile records")
                )
        ...
        if dry_run:
            ...
            self.stdout.write(
                f"   • Unstaked {total_unstaked} TEO from {len(students_with_staking)} students")
            self.stdout.write(f"   • All TEO moved to available balance")
            self.stdout.write(
                f"   • Students can now only use TEO for discounts")
            self.stdout.write(
                f"   • Only teachers can stake TEO for commission benefits")

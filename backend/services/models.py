...
class DiscountRequest(models.Model):
    ...
    teo_cost = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="TEO cost for discount")

    # Status tracking
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default='pending')
    student_signature = models.CharField(
        max_length=132, help_text="Student's message signature")
    platform_tx_hash = models.CharField(
        max_length=66, blank=True, help_text="Platform transaction hash")
    ...
    contract_request_id = models.PositiveIntegerField(
        null=True, blank=True, help_text="Contract request ID")
    gas_cost_matic = models.DecimalField(
        max_digits=10, decimal_places=6, null=True, blank=True, help_text="Platform gas cost in MATIC")
...
class StakingActivity(models.Model):
    ...
    teo_amount = models.DecimalField(
        max_digits=18, decimal_places=8, help_text="TEO amount involved")
    previous_tier = models.CharField(
        max_length=20, blank=True, help_text="Previous staking tier")
    new_tier = models.CharField(max_length=20, help_text="New staking tier")
    platform_tx_hash = models.CharField(
        max_length=66, help_text="Platform transaction hash")
    gas_cost_matic = models.DecimalField(
        max_digits=10, decimal_places=6, help_text="Platform gas cost in MATIC")
    created_at = models.DateTimeField(auto_now_add=True)
...
class PlatformGasExpense(models.Model):
    ...
    transaction_hash = models.CharField(
        max_length=66, unique=True, help_text="Transaction hash")
    gas_used = models.PositiveIntegerField(help_text="Gas units used")
    gas_price_gwei = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Gas price in Gwei")
    total_cost_matic = models.DecimalField(
        max_digits=10, decimal_places=6, help_text="Total cost in MATIC")
    total_cost_usd = models.DecimalField(
        max_digits=8, decimal_places=4, null=True, blank=True, help_text="Total cost in USD")
    related_address = models.CharField(
        max_length=42, blank=True, help_text="Related user address")
    created_at = models.DateTimeField(auto_now_add=True)
...
class PlatformAllowance(models.Model):
    ...
    student_address = models.CharField(
        max_length=42, unique=True, help_text="Student's wallet address")
    total_allowance = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="Total TEO allowance given")
    used_allowance = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, help_text="TEO allowance used")
    remaining_allowance = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="TEO allowance remaining")
    last_topped_up = models.DateTimeField(
        null=True, blank=True, help_text="Last top-up timestamp")
    approval_tx_hash = models.CharField(
        max_length=66, blank=True, help_text="Initial approval transaction")
    created_at = models.DateTimeField(auto_now_add=True)

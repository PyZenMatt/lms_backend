...
class CourseServiceTestCase(TestCase):
    ...
    def test_get_course_details_with_user(self):
        ...
        details = course_service.get_course_details(
            self.course.id, user=self.student)

        self.assertTrue(details['is_enrolled'])
        # 1 out of 2 lessons completed
        self.assertEqual(details['progress'], 50)

        # Check lesson completion status
        lesson_1_data = next(
            l for l in details['lessons'] if l['id'] == self.lesson1.id)
        lesson_2_data = next(
            l for l in details['lessons'] if l['id'] == self.lesson2.id)
    ...
    def test_get_student_enrollments(self):
        ...
        # 1 out of 2 lessons completed
        self.assertEqual(enrollment_data['progress'], 50)
    ...
    def test_calculate_course_progress_all_completed(self):
        ...
        LessonCompletion.objects.create(
            student=self.student, lesson=self.lesson2)
    ...
    def test_is_lesson_completed(self):
        ...
        self.assertFalse(course_service._is_lesson_completed(
            self.lesson1, self.student))
        ...
        self.assertTrue(course_service._is_lesson_completed(
            self.lesson1, self.student))

        # Test with no user
        self.assertFalse(
            course_service._is_lesson_completed(self.lesson1, None))

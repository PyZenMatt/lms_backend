...
class PaymentServiceTestCase(TestCase):
    ...
    def test_initiate_course_purchase_updates_wallet(self, mock_balance):
        ...
        payment_service.initiate_course_purchase(
            user_id=self.student.id, course_id=self.course.id, wallet_address=new_wallet
        )
    ...
    def test_complete_course_purchase_success(self, mock_verify):
        ...
        purchase_tx = BlockchainTransaction.objects.get(
            transaction_hash=tx_hash, transaction_type="course_purchase"
        )
        ...
        teacher_tx = BlockchainTransaction.objects.get(
            transaction_hash=tx_hash, transaction_type="course_earned"
        )
        ...
        commission_tx = BlockchainTransaction.objects.get(
            transaction_hash=tx_hash, transaction_type="platform_commission"
        )
    ...
    def test_get_user_purchase_history(self):
        ...
        purchase_data = next(
            tx for tx in history if tx["transaction_type"] == "course_purchase"
        )
    ...
    def test_get_user_purchase_history_with_filters(self):
        ...
        history = payment_service.get_user_purchase_history(
            self.student.id, transaction_type="course_purchase"
        )
        ...
        # Filter with limit
        history = payment_service.get_user_purchase_history(self.student.id, limit=1)
    ...
    def test_get_course_sales_stats(self):
        ...
        student2 = User.objects.create_user(
            username="student3", email="student3@test.com", role="student"
        )

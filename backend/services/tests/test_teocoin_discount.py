...
class TeoCoinDiscountServiceTests(TestCase):
    ...
    @patch(
        "services.teocoin_discount_service.TeoCoinDiscountService._execute_platform_transaction"
    )
    ...
    def test_create_discount_request_success(
        self, mock_reward_balance, mock_student_balance, mock_execute_tx
    ):
        ...
        with patch.object(
            self.service, "discount_contract"
        ) as mock_contract, patch.object(
            self.service, "platform_account"
        ) as mock_account:
            ...
            mock_receipt = {"logs": []}
            with patch(
                "services.teocoin_discount_service.TeoCoinDiscountService._extract_request_id_from_receipt",
                return_value=1,
            ), patch(
                "web3.Web3.eth.wait_for_transaction_receipt", return_value=mock_receipt
            ):
...
class PaymentAPITests(APITestCase):
    ...
    def test_payment_summary_endpoint(self):
        ...
        data = {"base_price": 50.00, "discount_percent": 10}
...
class NotificationServiceTests(TestCase):
    ...
    def setUp(self):
        ...
        # Create test users
        self.student = User.objects.create_user(
            username="notif_student", email="student@notif.com"
        )

        self.teacher = User.objects.create_user(
            username="notif_teacher", email="teacher@notif.com", role="teacher"
        )
    ...
    def test_teacher_discount_notification(self):
        ...
        notification = Notification.objects.filter(
            user=self.teacher, notification_type="teocoin_discount_pending"
        ).first()
...
class DatabaseModelTests(TestCase):
    ...
    def setUp(self):
        """Set up test data"""
        self.teacher = User.objects.create_user(
            username="db_teacher", email="teacher@db.com", role="teacher"
        )

        self.student = User.objects.create_user(
            username="db_student", email="student@db.com"
        )
...
class EndToEndFlowTests(TransactionTestCase):
    ...
    @patch("courses.views.payments.TeoCoinDiscountService.create_discount_request")
    @patch(
        "notifications.services.TeoCoinDiscountNotificationService.notify_teacher_discount_pending"
    )

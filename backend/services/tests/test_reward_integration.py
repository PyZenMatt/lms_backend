...
class RewardServiceIntegrationTestCase(TestCase):
    ...
    def test_complete_lesson_with_course_completion(self):
        ...
        response1 = self.client.post(
            self.lesson_complete_url, {"lesson_id": self.lesson1.id}
        )
        self.assertEqual(response1.status_code, status.HTTP_200_OK)

        # Complete second lesson (should trigger course completion)
        response2 = self.client.post(
            self.lesson_complete_url, {"lesson_id": self.lesson2.id}
        )
        ...
        self.assertTrue(
            Notification.objects.filter(
                user=self.student, notification_type="course_completion_bonus"
            ).exists()
        )
    ...
    def test_invalid_data_formats(self):
        ...
        response = self.client.post(self.lesson_complete_url, {"lesson_id": "invalid"})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # Test rewards summary with invalid time_period
        response = self.client.get(self.rewards_summary_url, {"time_period": "invalid"})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # Test leaderboard with invalid limit
        response = self.client.get(self.leaderboard_url, {"limit": "invalid"})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

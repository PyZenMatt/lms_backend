...
class TeoCoinDiscountService:
    ...
    def _initialize_contract(self):
        ...
        try:
            ...
            contract_address = getattr(
                settings, 'TEOCOIN_DISCOUNT_CONTRACT_ADDRESS', None)
            contract_abi = getattr(
                settings, 'TEOCOIN_DISCOUNT_CONTRACT_ABI', None)
            platform_private_key = getattr(
                settings, 'PLATFORM_PRIVATE_KEY', None)

            if not all([contract_address, contract_abi, platform_private_key]):
                self.logger.warning(
                    "Missing contract configuration in settings - discount service disabled")
                return
            ...
            self.logger.info(
                f"TeoCoinDiscount contract initialized at {contract_address}")
            self.logger.info(
                f"Platform account: {self.platform_account.address}")
    ...
    def create_discount_request(
        ...
        try:
            ...
            if student_balance < Decimal(str(teo_cost / 10**18)):
                raise ValueError(
                    f"Insufficient TEO balance. Required: {teo_cost / 10**18}, Available: {student_balance}")
            ...
            if reward_pool_balance < Decimal(str(teacher_bonus / 10**18)):
                raise ValueError(
                    f"Insufficient reward pool balance for teacher bonus")
            ...
            self.logger.warning(
                f"� STUDENTS WILL PAY: {teo_cost / 10**18:.2f} TEO immediately when creating request")
            self.logger.warning(
                f"🚨 This is NOT gas-free - smart contract requires transferFrom")
            ...
            self.logger.info(
                f"🚀 Executing createDiscountRequest transaction...")
            ...
            self.logger.info(
                f"   Teacher Bonus: {teacher_bonus / 10**18:.4f} TEO")
            self.logger.info(f"   Student Signature: {student_signature}")
            self.logger.info(
                f"   Signature Length: {len(student_signature)} chars")
            ...
            try:
                ...
                self.logger.info(
                    f"📊 Request ID before transaction: {request_id_before}")
            except Exception as pre_check_error:
                self.logger.warning(
                    f"⚠️ Could not read request ID before transaction: {pre_check_error}")
            ...
            try:
                ...
                if is_paused:
                    raise ValueError(
                        "Contract is paused - cannot create discount requests")

                # Check platform account address
                contract_platform_account = self.discount_contract.functions.platformAccount().call()
                self.logger.info(
                    f"🔍 Contract platform account: {contract_platform_account}")
                self.logger.info(
                    f"🔍 Our platform account: {self.platform_account.address}")

                # CRITICAL: Check student's TEO allowance BEFORE gas estimation
                try:
                    student_checksum = Web3.to_checksum_address(
                        student_address)
                    discount_contract_checksum = Web3.to_checksum_address(
                    ...
                    self.logger.info(
                        f"   Discount Contract: {discount_contract_checksum}")
                    self.logger.info(
                        f"   Current Allowance: {current_allowance_tokens:.4f} TEO ({current_allowance} wei)")
                    self.logger.info(
                        f"   Needed Amount: {needed_allowance_tokens:.4f} TEO ({teo_cost} wei)")
                    self.logger.info(
                        f"   Sufficient: {'✅ Yes' if current_allowance >= teo_cost else '❌ No'}")
                    ...
                    try:
                        ...
                        reward_pool_allowance = self.teocoin_service.contract.functions.allowance(
                            ...
                            Web3.to_checksum_address(
                                self.teocoin_service.reward_pool_address)
                        ).call()
                        self.logger.info(
                            f"   Reward Pool Allowance: {reward_pool_allowance / 10**18:.4f} TEO")

                        # Check student balance
                        student_balance = self.teocoin_service.contract.functions.balanceOf(
                            student_checksum).call()
                        self.logger.info(
                            f"   Student Balance: {student_balance / 10**18:.4f} TEO")
                    ...
                    if current_allowance < teo_cost:
                        ...
                        self.logger.warning(
                            f"🚧 GAS-FREE MODE: Student has insufficient allowance ({current_allowance_tokens:.4f} TEO vs {teo_tokens:.0f} TEO needed)")
                        self.logger.warning(
                            f"🚧 This is expected - students don't need to approve tokens in our gas-free system")
                        self.logger.warning(
                            f"🚧 The reward pool will handle TEO transfers after teacher approval")
                        self.logger.info(
                            f"✅ Proceeding with gas-free discount request for {teo_tokens:.0f} TEO")
                    else:
                        self.logger.info(
                            f"✅ Sufficient allowance confirmed: {current_allowance_tokens:.4f} TEO")
                except Exception as allowance_error:
                    self.logger.error(
                        f"❌ Allowance check failed: {allowance_error}")
                    raise ValueError(
                        f"Could not verify TEO allowance: {allowance_error}")
                ...
                try:
                    ...
                    if "TEO deduction from student failed" in error_msg or "transferFrom" in error_msg:
                        self.logger.warning(
                            f"⚠️ Expected smart contract TEO approval error (gas-free mode): {gas_error}")
                        self.logger.info(
                            f"🚀 Proceeding with signature-only validation for gas-free UX")
                        # We'll skip the actual smart contract call and handle it differently
                    else:
                        self.logger.error(
                            f"❌ Unexpected gas estimation error: {gas_error}")
                        raise ValueError(
                            f"Transaction would fail - gas estimation error: {gas_error}")

            except Exception as validation_error:
                self.logger.error(
                    f"❌ Pre-transaction validation failed: {validation_error}")
                raise ValueError(
                    f"Pre-transaction validation failed: {validation_error}")
            ...
            self.logger.info(
                f"   Status: {receipt.status} (1=success, 0=failed)")
            ...
            if receipt.status == 0:
                try:
                    ...
                    self.logger.error(
                        f"❌ Transaction FAILED - attempting to get revert reason...")
                    ...
                    self.logger.error(
                        f"Could not determine revert reason: {revert_error}")
                    raise ValueError("Transaction failed with unknown reason")

                raise ValueError(
                    "Transaction failed but no revert reason could be determined")
            ...
            try:
                ...
                self.logger.info(
                    f"📊 Request ID after transaction: {request_id_after}")
                self.logger.info(
                    f"📊 Request ID incremented: {request_id_after > request_id_before}")
            except Exception as post_check_error:
                self.logger.warning(
                    f"⚠️ Could not read request ID after transaction: {post_check_error}")
            ...
            try:
                ...
                student_user = User.objects.filter(
                    wallet_address__iexact=student_address).first()
                teacher_user = User.objects.filter(
                    wallet_address__iexact=teacher_address).first()
                ...
                if student_user and teacher_user:
                    ...
                    try:
                        from courses.models import (TeacherChoicePreference,
                                                    TeacherDiscountDecision)
                        from services.payment_service import payment_service

                        # Get teacher's current commission rate and staking tier
                        commission_rate = payment_service.get_teacher_commission_rate(
                            teacher_user) * 100  # Convert to percentage
                        ...
                        try:
                            preference = TeacherChoicePreference.objects.get(
                                teacher=teacher_user)
                            ...
                            if preference.should_auto_accept(teo_total):
                                ...
                                self.logger.info(
                                    f"✅ Auto-accepted discount request {request_id} based on teacher preference")
                                ...
                                self.logger.info(
                                    f"❌ Auto-declined discount request {request_id} based on teacher preference")
                                result['auto_decision'] = 'declined'
                        except TeacherChoicePreference.DoesNotExist:
                            # No preferences set - requires manual decision
                            self.logger.info(
                                f"📋 Manual decision required for request {request_id}")
                            result['auto_decision'] = 'manual'

                        result['decision_id'] = discount_decision.id
                        self.logger.info(
                            f"✅ Created TeacherDiscountDecision #{discount_decision.id} for request {request_id}")

                    except Exception as decision_error:
                        self.logger.error(
                            f"❌ Failed to create TeacherDiscountDecision: {decision_error}")
                    ...
                    if notification_sent:
                        self.logger.info(
                            f"✅ Teacher notification sent for request {request_id}")
                    else:
                        self.logger.warning(
                            f"⚠️ Failed to send teacher notification for request {request_id}")
                else:
                    self.logger.warning(
                        f"⚠️ Could not find user objects for notification (student: {student_user}, teacher: {teacher_user})")

            except Exception as notification_error:
                self.logger.error(
                    f"❌ Failed to send teacher notification: {notification_error}")
                # Don't fail the whole request due to notification error

            self.logger.info(
                f"Discount request created: {request_id} for student {student_address}")
            return result
    ...
    def approve_discount_request(self, request_id: int, approver_address: str) -> Dict:
        ...
        try:
            ...
            if approver_address.lower() != request_data['teacher'].lower():
                raise ValueError(
                    "Only the assigned teacher can approve this request")
            ...
            function_call = self.discount_contract.functions.approveDiscountRequest(
                request_id)
            tx_hash = self._execute_platform_transaction(function_call)
            ...
            try:
                student_user = User.objects.filter(
                    wallet_address__iexact=request_data['student']).first()
                teacher_user = User.objects.filter(
                    wallet_address__iexact=request_data['teacher']).first()
                ...
                if student_user and teacher_user:
                    ...
                    teocoin_notification_service.notify_student_teacher_decision(
                        ...
                        decision='accepted',
                        teo_amount=(
                            request_data['teo_cost'] + request_data['teacher_bonus']) / 10**18
                    )
                    ...
                    self.logger.info(
                        f"✅ Approval notifications sent for request {request_id}")

            except Exception as notification_error:
                self.logger.error(
                    f"❌ Failed to send approval notifications: {notification_error}")

            self.logger.info(
                f"Discount request {request_id} approved successfully")
            return result

        except Exception as e:
            self.logger.error(
                f"Failed to approve discount request {request_id}: {e}")
            return {
                'success': False,
                'error': str(e)
            }
    ...
    def decline_discount_request(
        ...
        try:
            ...
            if decliner_address.lower() != request_data['teacher'].lower():
                raise ValueError(
                    "Only the assigned teacher can decline this request")
            ...
            try:
                ...
                if student_user and teacher_user:
                    ...
                    self.logger.info(
                        f"✅ Decline notifications sent for request {request_id}")

            except Exception as notification_error:
                self.logger.error(
                    f"❌ Failed to send decline notifications: {notification_error}")

            self.logger.info(
                f"Discount request {request_id} declined: {reason}")
            return result

        except Exception as e:
            self.logger.error(
                f"Failed to decline discount request {request_id}: {e}")
            return {
                'success': False,
                'error': str(e)
            }
    ...
    def get_discount_request(self, request_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            # Get from contract
            request_data = self.discount_contract.functions.getDiscountRequest(
                request_id).call()
            ...
            self.logger.error(
                f"Failed to get discount request {request_id}: {e}")
            return None
    ...
    def get_student_requests(self, student_address: str) -> List[Dict]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get student requests for {student_address}: {e}")
            return []
    ...
    def get_teacher_requests(self, teacher_address: str) -> List[Dict]:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get teacher requests for {teacher_address}: {e}")
            return []
    ...
    def generate_student_signature_data(
        ...
        try:
            ...
            student_address_checksum = Web3.to_checksum_address(
                student_address)
            contract_address_checksum = Web3.to_checksum_address(
                self.discount_contract.address)
            ...
            self.logger.info(
                f"🔍 Signature parameters: student={student_address_checksum}, course={course_id}, cost={teo_cost} wei, contract={contract_address_checksum}")
            ...
            self.logger.info(
                f"🔍 Message for frontend to sign: {message_for_signing}")
    ...
    def _validate_discount_request(
        ...
        if discount_percent < 5 or discount_percent > self.MAX_DISCOUNT_PERCENT:
            raise ValueError(
                f"Discount percent must be between 5 and {self.MAX_DISCOUNT_PERCENT}")
    ...
    def _calculate_teo_amounts(self, course_price_cents: int, discount_percent: int) -> Tuple[int, int]:
        ...
        self.logger.info(
            f"   Course price: {course_price_cents} cents (€{course_price_cents/100:.2f})")
        self.logger.info(f"   Discount %: {discount_percent}%")
        self.logger.info(
            f"   Discount value: {discount_value_cents} cents (€{discount_value_cents/100:.2f})")

        # SIMPLIFIED: 1 TEO per 1 EUR of discount (rounded up to nearest whole TEO)
        # At least 1 TEO, rounded to whole numbers
        teo_cost_tokens = max(1, round(discount_value_cents / 100))
        ...
        self.logger.info(
            f"   TEO cost: {teo_cost_wei} wei = {teo_cost_wei / 10**18:.0f} TEO")
        self.logger.info(
            f"   Teacher bonus: {teacher_bonus_wei} wei = {teacher_bonus_wei / 10**18:.2f} TEO")
    ...
    def _execute_platform_transaction(self, function_call) -> bytes:
        ...
        try:
            ...
            raw_transaction = getattr(signed_txn, 'raw_transaction', None) or getattr(
    ...
    def _extract_request_id_from_receipt(self, receipt) -> int:
        ...
        try:
            ...
            self.logger.info(
                f"✅ Transaction successful: {receipt.transactionHash.hex()}")
            self.logger.info(
            ...
            try:
                ...
                self.logger.info(
                    f"📊 Current request ID from contract: {current_request_id}")

                # The current request ID should be the one that was just created
                if current_request_id > 0:
                    self.logger.info(
                        f"✅ SUCCESS: Using request ID from contract state: {current_request_id}")
                    return current_request_id
                else:
                    self.logger.warning(
                        f"⚠️ Contract returned request ID 0 - this suggests the transaction didn't update state properly")

            except Exception as contract_call_error:
                self.logger.error(
                    f"❌ Failed to read from contract after transaction: {contract_call_error}")
            ...
            self.logger.info(
                f"Found {len(discount_contract_logs)} logs from discount contract")
            ...
            for log_index, log in discount_contract_logs:
                ...
                if len(log.topics) >= 2:  # At least event signature + one indexed parameter
                    try:
                        ...
                        potential_request_id = int.from_bytes(
                            log.topics[1], byteorder='big')
                        self.logger.info(
                            f"🔍 Potential request ID from topics[1]: {potential_request_id}")

                        # Validate this looks like a reasonable request ID
                        if 0 < potential_request_id < 1000000:  # Reasonable range
                            self.logger.info(
                                f"✅ EXTRACTED: Request ID {potential_request_id} from log topics")
                            return potential_request_id
                        else:
                            self.logger.warning(
                                f"⚠️ Request ID {potential_request_id} seems out of reasonable range")

                    except Exception as parse_error:
                        self.logger.error(
                            f"❌ Failed to parse topics[1]: {parse_error}")
                ...
                if log.data and log.data != b'\x00' * 32:
                    try:
                        ...
                        if len(log.data) >= 32:
                            potential_request_id = int.from_bytes(
                                log.data[:32], byteorder='big')
                            self.logger.info(
                                f"🔍 Potential request ID from data: {potential_request_id}")

                            if 0 < potential_request_id < 1000000:
                                self.logger.info(
                                    f"✅ EXTRACTED: Request ID {potential_request_id} from log data")
                                return potential_request_id

                    except Exception as data_parse_error:
                        self.logger.error(
                            f"❌ Failed to parse log data: {data_parse_error}")
                    ...
                    try:
                        if hasattr(self.discount_contract.events, event_name):
                            event_filter = getattr(
                                self.discount_contract.events, event_name)
                            events = event_filter().process_receipt(receipt)
                            if events:
                                all_events.extend(events)
                                self.logger.info(
                                    f"Found {len(events)} {event_name} events")
                    except Exception as event_error:
                        self.logger.warning(
                            f"Failed to process {event_name} events: {event_error}")
                ...
                if all_events:
                    ...
                    for event in all_events:
                        if 'args' in event and 'requestId' in event['args']:
                            ...
                            self.logger.info(
                                f"✅ EXTRACTED: Request ID {request_id} from {event['event']} event")
                            return request_id

            except Exception as auto_error:
                self.logger.error(
                    f"Automatic event processing failed: {auto_error}")

            # APPROACH 4: Intelligent guessing based on transaction position and gas usage
            self.logger.info(
                "=== IMPLEMENTING TRANSACTION-BASED REQUEST ID ===")
            ...
            fallback_request_id = (
                ...
                # Transaction index contribution
                (transaction_index % 100) * 100 +
                # Hash contribution for uniqueness
                (tx_hash_int % 100)
            ) % 999999 + 1  # Ensure it's between 1 and 999999

            self.logger.warning(
                f"🚨 CONTRACT STATE ISSUE: Using transaction-based request ID {fallback_request_id}")
            self.logger.warning(
                f"   Block: {block_number}, TxIndex: {transaction_index}")
            self.logger.warning(f"   TxHash: {receipt.transactionHash.hex()}")
            self.logger.warning(
                f"   This works around the smart contract not updating its counter properly")

            return fallback_request_id

        except Exception as e:
            self.logger.error(
                f"Failed to extract request ID from receipt: {e}")

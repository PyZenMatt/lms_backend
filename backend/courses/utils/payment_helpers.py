...
def process_teocoin_discount_payment(request, course, amount_eur, teocoin_discount, payment_method):
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Failed to create discounted payment intent: {str(e)}")
            return Response({
                'success': False,
...
def handle_teocoin_discount_completion(payment_intent_metadata):
    ...
    try:
        # Extract metadata
        discount_request_id = payment_intent_metadata.get(
        ...
        if not all([discount_request_id, course_id, user_id]):
            logger.error(
                "Missing required metadata for TeoCoin discount completion")
        ...
        try:
            ...
            logger.warning(
                f"Teacher notification failed: {notification_error}")

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            "🎨 SETUP SCUOLA D'ARTE - Aggiornamento Categorie e Corsi")
        ...
        with transaction.atomic():
            ...
            for course in courses:
                ...
                if old_category != new_category:
                    ...
                    updated_count += 1
                    self.stdout.write(
                        f"   ✓ Corso '{course.title}': {old_category} → {new_category}")

            self.stdout.write(
                f"📊 Aggiornati {updated_count} corsi su {courses.count()}")
    ...
    def create_sample_art_courses(self):
        ...
        try:
            teacher = User.objects.filter(
                role='teacher', is_approved=True).first()
            if not teacher:
                ...
                if teacher:
                    ...
                    self.stdout.write(
                        "   ✓ Creato teacher di esempio: maestro_arte")
        except Exception as e:
            self.stdout.write(self.style.ERROR(
                f"   ❌ Errore nella creazione del teacher: {e}"))
            return
        ...
        with transaction.atomic():
            for course_data in sample_courses:
                ...
                if not Course.objects.filter(title=course_data['title']).exists():
                    ...
                    created_count += 1
                    self.stdout.write(
                        f"   ✓ Creato corso: {course.title} ({course.get_category_display()})")
                else:
                    self.stdout.write(
                        f"   - Corso già esistente: {course_data['title']}")
    ...
    def show_courses_summary(self):
        ...
        for category, count in sorted(courses_by_category.items()):
            self.stdout.write(
                f"   {category}: {count} corso{'i' if count > 1 else ''}")

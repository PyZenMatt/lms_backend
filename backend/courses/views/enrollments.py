...
class PurchaseCourseView(APIView):
        ...
        try:
            ...
            if not payment_confirmed or not transaction_hash:
                try:
                    ...
                    return Response(
                        {"error": str(e)}, status=getattr(e, "status_code", 400)
                    )
    ...
    def _verify_course_payment_transaction(
        self, tx_hash, student_address, teacher_address, course_price
    ):
        """
            ...
            if related_transactions.exists():
                ...
                for tx in related_transactions:
                    if (
                        tx.from_address
                        and tx.to_address
                        and tx.from_address.lower() == student_address.lower()
                        and tx.to_address.lower() == teacher_address.lower()
                        and abs(tx.amount) >= course_price * Decimal("0.85")
                    ):  # Teacher riceve almeno 85%
                        logger.info(
                            f"Found valid course payment transaction: {tx_hash}"
                        )
                        return True

            # Fallback: usa la logica blockchain standard
            return self._verify_blockchain_transaction(
                tx_hash, student_address, teacher_address, course_price
            )

        except Exception as e:
            logger.error(
                f"Error verifying course payment transaction {tx_hash}: {str(e)}"
            )
            return False

    def _verify_blockchain_transaction(
        self, tx_hash, from_address, to_address, expected_amount
    ):
        ...
        try:
            ...
            if tx_hash.startswith("0x") and len(tx_hash) == 66:
                ...
                if simulated_tx:
                    ...
                    simulated_from = (
                        simulated_tx.from_address.lower()
                        if simulated_tx.from_address
                        else ""
                    )
                    simulated_to = (
                        simulated_tx.to_address.lower()
                        if simulated_tx.to_address
                        else ""
                    )
                    ...
                    if (
                        simulated_from == request_from
                        and simulated_to == request_to
                        ...
                        logger.warning(
                            f"Simulated transaction details don't match: {tx_hash}"
                        )
                        return False
            ...
            if (
                tx_to
                and contract_addr
                and str(tx_to).lower() != str(contract_addr).lower()
            ):
            ...
            for event in transfer_events:
                ...
                if (
                    ...
                    and (to_address is None or event_to == to_address.lower())
                    and Decimal(str(event_amount)) >= expected_amount
                ):
                    logger.info(
                        f"Valid transfer found: {event_amount} TEO from {event_from} to {event_to}"
                    )
                    return True
...
class CourseEnrollmentView(APIView):
    ...
    def post(self, request, course_id):
        ...
        debug_messages.append(
            f"🔥 CourseEnrollmentView called - Course: {course_id}, User: {request.user.id}"
        )
        ...
        try:
            ...
            debug_messages.append(
                f"📊 Payment data: method={payment_method}, amount={amount}, discount_applied={discount_applied}"
            )
            ...
            with transaction.atomic():
                ...
                debug_messages.append(
                    f"✅ Student {request.user.id} added to course {course_id}"
                )
                ...
                enrollment_defaults = {
                    "payment_method": (
                        payment_method
                        if payment_method
                        in ["fiat", "teocoin", "teocoin_discount", "free", "admin"]
                        else "fiat"
                    ),
                ...
                if discount_applied and isinstance(discount_info, dict):
                    try:
                        discount_amount_eur = Decimal(
                            str(discount_info.get("discount_amount_eur", "0"))
                        )
                    except Exception:
                ...
                if stripe_payment_intent:
                    enrollment_defaults["stripe_payment_intent_id"] = str(
                        stripe_payment_intent
                    )

                enrollment_record, created = CourseEnrollment.objects.get_or_create(
                    student=request.user, course=course, defaults=enrollment_defaults
                )

                if created:
                    debug_messages.append(
                        f"✅ CourseEnrollment record created: {enrollment_record.pk}"
                    )
                else:
                    debug_messages.append(
                        f"ℹ️ CourseEnrollment record already exists: {enrollment_record.pk}"
                    )
                ...
                if discount_applied and isinstance(discount_info, dict):
                    try:
                        ...
                        if discount_amount_eur > 0:
                            pre_balance = db_teo_service.get_available_balance(
                                request.user
                            )
                ...
                if course.teocoin_reward > 0:
                    debug_messages.append(
                        f"🎁 Attempting TeoCoin reward: {course.teocoin_reward} TEO"
                    )
                    try:
                        db_teo_service = DBTeoCoinService()
                        ...
                        if reward_ok:
                            debug_messages.append(
                                f"✅ TeoCoin reward granted: {course.teocoin_reward} TEO"
                            )
                        else:
                            debug_messages.append(
                                "❌ TeoCoin reward failed: service returned False"
                            )
                    except Exception as e:
                ...
                if discount_applied and discount_info and course.teacher:
                    ...
                    try:
                        ...
                        absorption_data = {
                            ...
                            "discount_percentage": discount_info.get(
                                "discount_percentage", 0
                            ),
                            "teo_used": discount_info.get("teo_used", 0),
                            "discount_amount_eur": discount_info.get(
                        ...
                        debug_messages.append(
                            f"✅ Discount absorption opportunity created: {absorption.pk}"
                        )

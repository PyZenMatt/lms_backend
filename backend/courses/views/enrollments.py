        ...
        try:
            ...
            if tx_hash.startswith("0x") and len(tx_hash) == 66:
                ...
                if simulated_tx:
                    ...
                    simulated_from = simulated_tx.from_address.lower(
                    ) if simulated_tx.from_address else ""
                    ...
                    if (simulated_from == request_from and
                        ...
                        logger.info(
                            f"Valid simulated transaction found: {tx_hash}")
                        return True
                    else:
                        logger.warning(
            ...
            if tx_to and contract_addr and str(tx_to).lower() != str(contract_addr).lower():
                logger.warning(
                    f"Transaction not to TeoCoin contract: {tx_hash}")
                return False
            ...
            for event in transfer_events:
                ...
                event_amount = teocoin_service.w3.from_wei(
                    event.args['value'], 'ether')
            ...
            logger.warning(
                f"No valid transfer event found in transaction: {tx_hash}")
            return False
...
class StudentEnrolledCoursesView(APIView):
    ...
    def get(self, request):
        ...
        enrolled_courses = Course.objects.filter(
            students=student, is_approved=True)
...
class TeacherCourseStudentsView(APIView):
    ...
    def get(self, request):
        ...
        courses = Course.objects.filter(
            teacher=teacher).prefetch_related('students')
            ...
            with transaction.atomic():
                ...
                if discount_applied and discount_info and course.teacher:
                    ...
                    try:
                        ...
                        debug_messages.append(
                            f"❌ Discount absorption failed: {e}")

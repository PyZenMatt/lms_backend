...
class LessonViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        ).prefetch_related("exercises", "completions")

    @action(
        detail=True,
        methods=["post"],
        url_path="mark_complete",
        url_name="mark-complete",
        permission_classes=[IsAuthenticated],
    )
    def mark_complete(self, request, pk=None):
        ...
        if not course:
            return Response(
                {"detail": "Lezione non associata a nessun corso."},
...
class LessonCreateAssignView(APIView):
    ...
    def post(self, request):
        ...
        if not course_id:
            return Response(
                {"error": "L'ID del corso è obbligatorio."},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class LessonDetailView(RetrieveAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (
            course
            and not (user.is_staff or user.is_superuser or course.teacher == user)

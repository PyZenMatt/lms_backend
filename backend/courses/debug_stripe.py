            ...
            return Response(
                {
                    ...
                    "stripe_secret_key_prefix": (
                        stripe_secret[:15] if stripe_secret else "NOT_SET"
                    ),
                    "stripe_publishable_key_prefix": (
                        stripe_publishable[:15] if stripe_publishable else "NOT_SET"
                    ),
                    "test_payment_intent_id": test_intent.id,
                    "stripe_api_key_set": (
                        stripe.api_key[:15] if stripe.api_key else "NOT_SET"
                    ),
                }
            )
            ...
            return Response(
                {
                    "success": False,
                    "error": str(e),
                    "stripe_secret_key_prefix": getattr(
                        settings, "STRIPE_SECRET_KEY", "NOT_SET"
                    )[:15],
                    "stripe_api_key_set": (
                        getattr(stripe, "api_key", "NOT_SET")[:15]

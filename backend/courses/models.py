...
class Course(models.Model):
    ...
    is_approved = models.BooleanField(
        default=False, help_text="Il corso deve essere approvato da un admin prima di essere messo in vendita.")
    ...
    def get_teocoin_price(self):
        ...
        if self.price_eur > 0:
            ...
            discount = base_teo_price * \
                (self.teocoin_discount_percent / Decimal('100'))
    ...
    def get_teocoin_discount_amount(self):
        ...
        if self.price_eur > 0 and self.teocoin_discount_percent > 0:
            ...
            discount_value_eur = (
                self.price_eur * self.teocoin_discount_percent) / Decimal('100')
...
class Lesson(models.Model):
    ...
    course = models.ForeignKey('Course', on_delete=models.CASCADE,
                               related_name='lessons_in_course', null=True, blank=True)
...
class Exercise(models.Model):
    ...
    def clean(self):
        ...
        if self.score and (self.score < 0 or self.score > 100):
            raise ValidationError(
                {'score': 'Il punteggio deve essere tra 0 e 100'})
    ...
    def reward_reviewer(review):
        ...
        if review.reviewer.wallet_address:
            try:
                ...
                logger = logging.getLogger(__name__)
                logger.error(
                    f"Failed to mint reward for review {review.pk}: {str(e)}")
        else:
...
class ReviewerReputation(models.Model):
    ...
    def update_reputation(reviewer, submission, score):
        average_score = submission.reviews.exclude(
            reviewer=reviewer).aggregate(models.Avg('score'))['score__avg']
        if average_score:
            ...
            reputation.reputation_score = max(
                1, reputation.reputation_score - penalty)
            reputation.save()

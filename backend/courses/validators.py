...
def validate_video_file(file):
    ...
    if file.size > max_size:
        raise ValidationError(
            f'Il file video è troppo grande. Massimo {max_size // (1024*1024)}MB permessi.')
    ...
    if file_extension not in valid_extensions:
        raise ValidationError(
            f'Formato file non supportato. Estensioni permesse: {", ".join(valid_extensions)}')
    ...
    if hasattr(file, 'content_type') and file.content_type:
        if not any(file.content_type.startswith(mime) for mime in valid_mime_types):
            raise ValidationError(
                f'Tipo MIME non supportato: {file.content_type}')

...
class BurnDepositView(APIView):
    ...
    def verify_burn_transaction(self, tx_hash: str, expected_amount: Decimal, expected_address: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"📊 Decoded function: {function_obj.function_identifier}")
                ...
                logger.warning(
                    f"⚠️ Could not decode transaction input: {decode_error}")
                ...
                logger.info(
                    f"⚠️ Using relaxed verification - transaction appears valid")
                return {
                    'valid': True,
                    'block_number': receipt['blockNumber'],
                    'gas_used': receipt['gasUsed'],
    ...
    def verify_burn_via_events(self, receipt, expected_amount: Decimal, expected_address: str) -> dict:
        ...
        try:
            ...
            import traceback
            logger.error(
                f"📜 Events verification traceback: {traceback.format_exc()}")

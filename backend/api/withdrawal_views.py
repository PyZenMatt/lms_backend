...
class UserWithdrawalHistoryView(APIView):
    ...
    def get(self, request):
        ...
        try:
            logger.info(
                f"📊 Withdrawal history request for {request.user.email}")
            ...
            if not hasattr(teocoin_withdrawal_service, 'get_user_withdrawal_history'):
                logger.error(
                    "❌ TeoCoin withdrawal service not properly initialized")
                return Response({
                    'success': False,
    ...
    def post(self, request, withdrawal_id):
        ...
        try:
            ...
            if result['success']:
                ...
                withdrawal.status = 'completed'
                withdrawal.transaction_hash = result.get(
                    'transaction_hash', 'pending')
                withdrawal.save()
...
class MintToAddressView(APIView):
    ...
    permission_classes = [
        IsAuthenticated]  # For now, authenticated users can use (for testing)

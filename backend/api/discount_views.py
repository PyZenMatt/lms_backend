    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                return self.json_response(
                    {
                        ...
                        "discount_value": float(
                            course_price_decimal * int(discount_percent) / 100
                        ),
                    }
                )
                ...
                return self.error_response(
                    "Encoding error in signature generation. Please check wallet address format."
                )
...
class CreateDiscountRequestView(BaseDiscountView):
    ...
    def post(self, request):
        try:
            ...
            logger.info(
                f"Raw parameters - student_address: {repr(data.get('student_address'))}, teacher_address: {repr(data.get('teacher_address'))}, course_id: {repr(data.get('course_id'))}"
            )
            logger.info(
                f"Parsed parameters - student_address: '{student_address}', teacher_address: '{teacher_address}', course_id: {course_id}, course_price: {course_price}, discount_percent: {discount_percent}, student_signature: '{student_signature[:20]}...' if student_signature else 'None'"
            )
            ...
            if not all(
                ...
                if not student_address:
                    missing_params.append("student_address")
                if not teacher_address:
                    missing_params.append("teacher_address")
                if not course_id:
                    missing_params.append("course_id")
                if not course_price:
                    missing_params.append("course_price")
                if not discount_percent:
                    missing_params.append("discount_percent")
                if not student_signature:
                    missing_params.append("student_signature")
            ...
            if result["success"]:
                logger.info(
                    f"Discount request created successfully: {result['request_id']}"
                )
                ...
                try:
                    ...
                    teo_cost, teacher_bonus = (
                        teocoin_discount_service.calculate_teo_cost(
                    ...
                    django_request = TeoCoinDiscountRequest.objects.create(
                        ...
                        course_id=course_id_int,
                        # Convert to cents
                        course_price=int(course_price_decimal * 100),
                        # Convert to basis points (10% = 1000)
                        discount_percent=discount_percent_int * 100,
                        teo_cost=int(teo_cost),  # TEO in wei
                        teacher_bonus=int(teacher_bonus),  # TEO in wei
                        expires_at=timezone.now()
                        + timedelta(hours=2),  # 2 hour expiration
                        blockchain_tx_hash=result.get("transaction_hash", ""),
                        status=0,  # Pending
                    )

                    logger.info(
                        f"✅ Django model record created: TeoCoinDiscountRequest ID {django_request.id}"
                    )
                    ...
                    try:
                        ...
                        logger.error(
                            f"⚠️ Teacher notification failed: {notification_error}"
                        )
                ...
                return self.error_response(
                    result.get("error", "Failed to create request"), status=400
                )
    ...
    def post(self, request):
        try:
            ...
            result = teocoin_discount_service.approve_discount_request(
                request_id=request_id_int, approver_address=approver_address
            )

            if result["success"]:
                logger.info(
                    f"Discount request {request_id_int} approved by {approver_address}"
                )
                return self.json_response(result)
            else:
                return self.error_response(
                    result.get("error", "Failed to approve request"), status=400
                )
...
class DeclineDiscountRequestView(BaseDiscountView):
    ...
    def post(self, request):
        try:
            ...
            if result["success"]:
                logger.info(
                    f"Discount request {request_id_int} declined by {decliner_address}"
                )
                return self.json_response(result)
            else:
                return self.error_response(
                    result.get("error", "Failed to decline request"), status=400
                )
...
class DiscountRequestDetailView(BaseDiscountView):
    ...
    def get(self, request, request_id):
        try:
            ...
            if request_data:
                return self.json_response({"success": True, "request": request_data})
            else:
...
class StudentRequestsView(BaseDiscountView):
    ...
    def get(self, request, student_address):
        try:
            ...
            return self.json_response(
                {"success": True, "requests": requests, "count": len(requests)}
            )
    ...
    def post(self, request):
        try:
            ...
            if not all([course_price, discount_percent]):
                return self.error_response(
                    "Course price and discount percentage required"
                )
...
class SystemStatusView(BaseDiscountView):
    ...
    def get(self, request):
        try:
            ...
            service_status = {
                "contract_initialized": teocoin_discount_service.discount_contract
                is not None,
                "platform_account_initialized": teocoin_discount_service.platform_account
                is not None,
                "web3_connected": (
                    teocoin_discount_service.w3.is_connected()
                    if teocoin_discount_service.w3
                    else False
                ),
            }
            ...
            teocoin_status = {
                ...
                "reward_pool_balance": (
                    float(teocoin_service.get_reward_pool_balance())
                    if teocoin_service
                    else 0
                ),
            }
            ...
            return self.json_response(
                {
                    "success": True,
                    "status": (
                        "operational" if all(service_status.values()) else "limited"
                    ),

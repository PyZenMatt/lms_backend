...
class StakingInfoView(APIView):
    ...
    def get(self, request):
        ...
        try:
                ...
                tier_info.update(
                    {
                        ...
                        "tier_name": (
                            profile.get_staking_tier_display()
                            if hasattr(profile, "get_staking_tier_display")
                            else "Bronze"
                        ),
            ...
            return Response({"success": True, "staking_info": tier_info})

        except Exception as e:
            logger.error(f"Error getting staking info for user {request.user.id}: {e}")
            return Response(
                {"success": False, "error": "Failed to get staking information"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

    ...
    def get(self, request):
        try:
            ...
            is_teacher = (
                ...
                or hasattr(request.user, "created_courses")
                and request.user.created_courses.exists()
            )
            ...
            return Response(
                {"success": False, "error": "Failed to fetch pending absorptions"},
    ...
    def post(self, request):
        ...
        try:
            ...
            try:
                ...
                return Response(
                    {"success": False, "error": "Missing or invalid absorption_id"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if getattr(absorption, "is_expired", False):
                ...
                return Response(
                    {"success": False, "error": "This opportunity has expired"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            if absorption.status != "pending":
                return Response(
                    {
                        "success": False,
                        "error": f"Already processed with status {absorption.status}",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            data = {
                ...
                "absorption": {
                    ...
                    "decided_at": (
                        processed.decided_at.isoformat()
                        if processed.decided_at
                        else None
                    ),
                },
            }
            ...
            return Response(
                {"success": False, "error": f"Processing error: {e}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
                ...
                history_data.append(
                    {
                        ...
                        "teo_used": float(absorption.teo_used_by_student),
                        "final_teacher_eur": (
                            float(absorption.final_teacher_eur)
                            if absorption.final_teacher_eur
                            else 0
                        ),
                        "final_teacher_teo": float(absorption.final_teacher_teo),
                        "created_at": absorption.created_at.isoformat(),
                        "decided_at": (
                            absorption.decided_at.isoformat()
                            if absorption.decided_at
                            else None
                        ),
                    }
                )

            return Response({"success": True, "history": history_data, "stats": stats})

        except Exception as e:
            logger.error(f"Error fetching absorption history: {str(e)}")
            return Response(
                {"success": False, "error": "Failed to fetch absorption history"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class AdminAbsorptionOverviewView(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response(
                {"success": False, "error": "Failed to fetch absorption overview"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

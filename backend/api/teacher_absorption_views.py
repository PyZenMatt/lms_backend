    ...
    def get(self, request):
        try:
            ...
            is_teacher = (
                ...
                hasattr(
                    request.user, 'created_courses') and request.user.created_courses.exists()
            )
...
class TeacherMakeAbsorptionChoiceView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            is_teacher = (getattr(request.user, 'role', None)
                          == 'teacher') or request.user.is_staff
            if not is_teacher:
            ...
            svc_choice = 'absorb' if raw_choice in (
                'teo', 'absorb') else 'refuse'
...
class TeacherAbsorptionHistoryView(APIView):
    ...
    def get(self, request):
        try:
            ...
            history = service.get_teacher_absorption_history(
                request.user, days)

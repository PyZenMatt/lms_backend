...
def test_complete_exercise_system():
    ...
    teacher = (
        User.objects.filter(role="teacher").first()
    ...
    BlockchainRewardCalculator.calculate_course_reward_pool(course)
    ...
    if not existing_reward:
        ...
        ), "Student balance did not increase after reward"
        submission.refresh_from_db()
        assert (
            submission.reward_amount is not None
        ), "Submission reward_amount not updated"
    ...
    if teacher:
        ...
        if review and submission.reward_amount and submission.reward_amount > 0:
            ...
            if not existing_review_reward:
                ...
                assert (
                    reviewer_new > reviewer_initial
                ), "Reviewer balance did not increase after reward"
